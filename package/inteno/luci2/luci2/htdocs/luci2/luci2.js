...
function LuCI2()
{
	...
	this.ui = {
		...
		renderViewMenu: function()
		{
			$('#viewmenu')
				.empty()
				.append(L.globals.mainMenu.render(1, 2));
		},
	...
	this.ui.view = this.ui.AbstractWidget.extend({
		...
		render: function()
		{
			...
			if (this.title)
				container.append($('<h1 />').append(this.title));
	...
	this.ui.menu = this.ui.AbstractWidget.extend({
		...
		// Populate menu
		// go through the nodes in the dispatcher tree
		//
		renderNodes: function(childs, level, min, max)
		{
			...
			if (v)
				currentMenu= v.substr(0, v.indexOf('/'));
			nodes.sort(this.sortNodesCallback);

			var list = $('<ul />');

			if (level == 0) // top-level menu
				list.addClass('clearfix '+currentMenu+'Menu')
			//else if (level == 1) // side menu
				//list.addClass('dropdown-menu').addClass('navbar-inverse');		
			$("#mainmenu ul:first").removeClass();
			$("#mainmenu ul:first").addClass('clearfix '+currentMenu+'Menu');
			for (var i = 0; i < nodes.length; i++)
			{
		...
		render: function(min, max)
		{
			...
			var node = L.globals.defaultNode;			
			if (name && L.globals.mainMenu)
				L.globals.defaultNode = L.globals.mainMenu.getNode(name);
			...
			else
			{
				for (var i = 0; i < this._rows.length; i++)
				{
					...
					for (var j = 0; j < this.options.columns.length; j++)
					{
						...
						if (col.verticalAlign)
							td.style.verticalAlign = col.verticalAlign;
